...
describe('Medusa', function() {
  ...
  it('should unstone the the first victim and stone the 4th coming in', function() {
    // create medusa
    var medusa = new Medusa('Jezzebelle')
    // create first victim
    var victim1 = new Person('Francois')
    // create second victim
    var victim2 = new Person('Pierre')
    // create third victim
    var victim3 = new Person('Emilio')
    // create fourth victim
    var victim4 = new Person('Jerry')
    ...
    assert.equal(medusa.statues[2].name, 'Jerry');
    // assert that the first victim is not stoned
    assert.equal(medusa.statues[0].name, 'Pierre');
  });
})
